    ...
    test("formatMainMethods"){
      ...
      val expected =
        """Mill Build Tool
          ...
          |  --repl                      Run Mill in interactive mode and start a build REPL. In this mode, no
          |                              mill server will be used. Must be the first argument.
          |  --no-server                 Run Mill in interactive mode, suitable for opening REPLs and taking
          |                              user input. In this mode, no mill server will be used. Must be the
          |                              first argument.
          |  -i --interactive            Run Mill in interactive mode, suitable for opening REPLs and taking
          |                              user input. In this mode, no mill server will be used. Must be the
          |                              first argument.
          |  -v --version                Show mill version and exit.
          |  -b --bell                   Ring the bell once if the run completes successfully, twice if it
          |                              fails.
          |  --disable-ticker            Disable ticker log (e.g. short-lived prints of stages and progress
          |                              bars)
          ...
          |  -j --jobs <int>             Allow processing N targets in parallel. Use 1 to disable parallel and
          |                              0 to use as much threads as available processors.
          ...
          |  -p --predef <path>          Lets you load your predef from a custom location, rather than the
          |                              default location in your Ammonite home
          |  --no-home-predef <bool>     Disables the default behavior of loading predef files from your
          |                              ~/.ammonite/predef.sc, predefScript.sc, or predefShared.sc. You can
          |                              choose an additional predef to use using `--predef
          |  --no-default-predef <bool>  Disable the default predef and run Ammonite with the minimal predef
          |                              possible
          |  -s --silent <bool>          Make ivy logs go silent instead of printing though failures will still
          |                              throw exception
          |  --color <bool>              Enable or disable colored output; by default colors are enabled in
          |                              both REPL and scripts if the console is interactive, and disabled
          |                              otherwise
          |  -w --watch <bool>           Watch and re-run your scripts when they change
          |  --bsp <bool>                Run a BSP server against the passed scripts
          |  --thin <bool>               Hide parts of the core of Ammonite and some of its dependencies. By
          |                              default, the core of Ammonite and all of its dependencies can be seen
          |                              by users from the Ammonite session. This option mitigates that via
          |                              class loader isolation.
          |  -b --banner <str>           Customize the welcome banner that gets shown when Ammonite starts
          |  --class-based <bool>        Wrap user code in classes rather than singletons, typically for Java
