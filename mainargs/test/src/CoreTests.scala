  ...
  val tests = Tests {
    ...
    test("basicModelling") {
      ...
      val evaledArgs = check.mains.value.map(_.argSigs.map {
        case ArgSig.Simple(name, s, docs, None, parser, _, _) => (s, docs, None, parser)
        case ArgSig.Simple(name, s, docs, Some(default), parser, _, _) =>
      ...
      test("missingParams") {
        test - assertMatch(check.parseInvoke(List("bar"))) {
          case Result.Failure.MismatchedArguments(
                Seq(ArgSig.Simple(None, Some('i'), _, _, _, _, _)),
                Nil,
                Nil,
                None
              ) =>
...
object CorePositionalDisabledOnlyTests extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("redundantParams") - check(
      List("qux", "1", "-i", "2"),
      MismatchedArguments(
        missing = List(ArgSig.Simple(None, Some('i'), None, None, TokensReader.IntRead, positional = false, isHidden = false)),
        unknown = List("1", "-i", "2")
      )
    )
  }
      ...
      test("invalidParams") - assertMatch(
        ...
        case Result.Failure.InvalidArguments(
              List(Result.ParamError.Failed(
                ArgSig.Simple(None, Some('i'), _, _, _, _, _),
                Seq("lol"),
                _
              ))
            ) =>
      ...
      test("redundantParams") {
        ...
        assertMatch(parsed) {
          case Result.Failure.MismatchedArguments(
                Nil,
                Nil,
                Seq((ArgSig.Simple(None, Some('i'), _, _, _, _, _), Seq("1", "2"))),
                None
              ) =>
        }
      }
