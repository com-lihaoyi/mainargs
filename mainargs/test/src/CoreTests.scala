  ...
  val tests = Tests {
    ...
    test("basicModelling") {
      ...
      val evaledArgs = check.mains.value.map(_.flattenedArgSigs.map {
        case ArgSig(name, s, docs, None, parser, _) => (s, docs, None, parser)
        case ArgSig(name, s, docs, Some(default), parser, _) =>
      ...
      test("missingParams") {
        test - assertMatch(check.parseInvoke(List("bar"))) {
          case Result.Failure.MismatchedArguments(
                Seq(ArgSig(None, Some('i'), _, _, _, _)),
                Nil,
                Nil,
                None
              ) =>
  ...
  val tests = Tests {
    test("invoke") {
      test - check(
        List("bar", "2"),
        MismatchedArguments(
          missing = List(ArgSig(None, Some('i'), None, None, TokensReader.IntRead, false)),
          unknown = List("2")
        )
      )
      test - check(
      ...
      test("redundantParams") {
        ...
        assertMatch(parsed) {
          case Result.Failure.MismatchedArguments(
                Nil,
                Nil,
                Seq((ArgSig(None, Some('i'), _, _, _, _), Seq("1", "2"))),
                None
              ) =>
        }
      }
