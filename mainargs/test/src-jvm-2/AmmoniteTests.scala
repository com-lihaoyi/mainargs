    ...
    test("formatMainMethods.unsorted") {
      ...
      val expected =
        """Ammonite REPL & Script-Runner
          ...
          |                         possible
          |  [-s --silent]          Make ivy logs go silent instead of printing though failures will still
          |                         throw exception
          ...
          |                         REPL and scripts if the console is interactive, and disabled otherwise
          |  [--thin]               Hide parts of the core of Ammonite and some of its dependencies. By
          |                         default, the core of Ammonite and all of its dependencies can be seen by
          |                         users from the Ammonite session. This option mitigates that via class
          |                         loader isolation.
          ...
          |                         ~/.ammonite/predef.sc, predefScript.sc, or predefShared.sc. You can choose
          |                         an additional predef to use using `--predef
          |  [-b --banner <str>]    Customize the welcome banner that gets shown when Ammonite starts
          |  [--no-remote-logging]  (deprecated) Disable remote logging of the number of times a REPL starts
          |                         and runs commands
