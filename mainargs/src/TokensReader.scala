...
object TokensReader {
  ...
  implicit object BooleanRead
      extends TokensReader[Boolean]("bool", strs => tryEither(strs.last.toBoolean))
  implicit object ByteRead extends TokensReader[Byte]("byte", strs => tryEither(strs.last.toByte))
  implicit object ShortRead
      extends TokensReader[Short]("short", strs => tryEither(strs.last.toShort))
  ...
  implicit def SeqRead[C[_] <: Iterable[_], T: TokensReader](implicit
      factory: Factory[T, C[T]]
  ): TokensReader[C[T]] = new TokensReader[C[T]](
    implicitly[TokensReader[T]].shortName,
    strs => {
      strs
